...
public class UserAOImpl implements IUserAO {
    ...
    // * companyCode, String systemCode, String unionId, String appOpenId,
    // String
    // * h5OpenId, String nickname, String photo, String userKind) { XN627302Res
    // * result; if (StringUtils.isNotBlank(mobile)) { // 判断是否需要验证码验证码,登录前一定要验证
    // if
    ...
    // * result = new XN627302Res(null, EBoolean.YES.getCode()); } return
    // result;
    // * }
    // */
    //
    ...
    // User user = userBO.getAgent(userId);
    // if (user == null) {
    ...
    // // 设置交易密码
    // @Override
    // public void setTradePwd(String userId, String smsCaptcha, String
    // tradePwd) {
    // User user = userBO.getCheckUser(userId);
    ...
    //
    // @Override
    // public Paginable<User> queryUserPage(int start, int limit, User
    // condition) {
    //
    ...
    // User toUser = userBO.getAgent(log.getToUserId());
    ...
    // User userRefeere = userBO.getAgent(userId);
    // if (userRefeere != null) {
    ...
    // user.setHighUser(highUser);
    // }
    // user.setToTeamName(highUser.getTeamName());
    // user.setToLevel(highUser.getLevel());
    // user.setToUserName(highUser.getRealName());
    // user.setToUserMobile(highUser.getToUserMobile());
    // }
    //
    // // 介绍人
    // if (StringUtils.isNotBlank(user.getIntroducer())) {
    ...
    // if (StringUtils.isNotBlank(user.getUserReferee())) {
    // // 有推荐人的时候，同步推荐人的团队等信息
    // User userReferee = userBO.getAgent(user.getUserReferee());
    ...
    // }
    //
    // User data = userBO.getAgent(req.getUserId());
    // data.setApplyLevel(StringValidater.toInteger(req.getApplyLevel()));
    ...
    // User userReferee = userBO.getAgent(data.getUserReferee());
    //
    ...
    // addressBO.saveAddress(data.getUserId(),
    // EAddressType.User_Address.getCode(), req.getMobile(),
    // req.getRealName(), req.getProvince(), req.getCity(),
    // req.getArea(), req.getAddress(), EBoolean.YES.getCode());
    // }
    ...
    // String approver) {
    // User data = userBO.getAgent(userId);
    // String status = EUserStatus.TO_APPROVE.getCode();
    ...
    // User toUser = userBO.getAgent(toUserId);
    ...
    // String logCode = agencyLogBO.acceptIntention(log, data.getStatus());
    // data.setLastAgentLog(logCode);
    ...
    //
    // // 等级不能高于上级
    // User highUser = userBO.getAgent(data.getUserId());
    ...
    // User user = userBO.getAgent(data.getHighUserId());
    // if (!EUserKind.Plat.getCode().equals(user.getKind())) {
    ...
    // @Override
    // @Transactional
    // public boolean approveImpower(String userId, String approver, String
    // result,
    // String remark) {
    // User data = userBO.getAgent(userId);
    // AgencyLog log = agencyLogBO.getAgencyLog(data.getLastAgentLog());
    ...
    // } else {
    // approveUser = userBO.getAgent(approver);
    // }
    //
    ...
    // // 审核通过，确定上级，等级，授权时间
    // if (StringUtils.isNotBlank(log.getToUserId())) {
    // highUser = userBO.getAgent(log.getToUserId());
    ...
    //
    // // 查看是否有账户（重新申请）
    // List<Account> accountList = accountBO
    // .getAccountByUser(data.getUserId());
    ...
    // User user = userBO.getAgent(data.getIntroducer());
    ...
    // ECurrency.MK_CNY.getCode());
    // // 该等级对应的郑策
    // AgentLevel impower = agentLevelBO.getAgentByLevel(data.getLevel());
    //
    // // 需要公司授权且审核人不是平台
    // if (EBoolean.YES.getCode().equals(impower.getIsCompanyImpower())
    // && EUserKind.Merchant.getCode()
    ...
    // @Override
    // @Transactional
    // public void editHighUser(String userId, String highUserId, String
    // updater) {
    // User data = userBO.getAgent(userId);
    // User newHighUser = userBO.getAgent(highUserId);
    //
    ...
    // // 旧上级非空（平台）
    // if (StringUtils.isNotBlank(data.getHighUserId())) {
    // User oldHighUser = userBO.getAgent(data.getHighUserId());
    // // 旧上级是平台
    // if (EUserKind.Plat.getCode().equals(newHighUser.getKind())) {
    // Account newHighAccount = accountBO.getAccountByUser(
    // newHighUser.getUserId(), ECurrency.MK_CNY.getCode());
    ...
    // String updater) {
    // User data = userBO.getAgent(userId);
    // User defereeData = userBO.getAgent(userReferee);
    ...
    // if (StringUtils.isNotBlank(data.getHighUserId())) {
    // User highUser = userBO.getAgent(data.getHighUserId());
    // if (EUserKind.Merchant.getCode().equals(highUser.getKind())) {
    ...
    // if (!EUser.ADMIN.getCode().equals(approver)) {
    // User approveUser = userBO.getAgent(approver);
    ...
    // if (StringUtils.isNotBlank(data.getHighUserId())) {
    // highUser = userBO.getAgent(data.getHighUserId());
    // data.setHighUser(highUser);
    // }
    // }
    //
    ...
    // User data = userBO.getAgent(condition.getUserId());
    ...
    // User user = userBO.getAgent(agencyLog.getApplyUser());
    // if (user != null) {
    // agencyLog.setUser(user);
    // if (StringUtils.isNotBlank(user.getUserReferee())) {
    // userReferee = userBO.getAgent(user.getUserReferee());
    // }
    // if (userReferee != null) {
    ...
    // User aprrvoeName = userBO.getAgent(agencyLog.getApprover());
    // if (null != aprrvoeName) {
    // userReferee = userBO.getAgent(aprrvoeName.getUserId());
    // if (userReferee != null) {
    ...
    // if (StringUtils.isNotBlank(user.getHighUserId())) {
    // User highUser = userBO.getAgent(user.getHighUserId());
    // if (EUserKind.Merchant.getCode().equals(highUser.getKind())) {
    ...
    // User highUser = userBO.getAgent(highUserId);
    // if (level != highUser.getLevel()) {
    // highUser = userBO.getAgent(highUser.getHighUserId());
